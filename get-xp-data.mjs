...
const CHARACTER_NAME = 'Sofsterella';
	...
	for (const [date, entry] of Object.entries(history)) {
		buffer.push(
			`\t${JSON.stringify(date)}: { "rank": ${
				...
				pad(entry.level, 3)}, "experience": ${
				pad(entry.experience, 10)} }`
		);
	}
	const json = `{\n${buffer.join(',\n')}\n}\n`;
	return json;
...
const getHighscoreData = async (page = 1) => {
	const url = `https://api.tibiadata.com/v4/highscores/Vunira/experience/sorcerers/${page}`;
	...
	if (page === 1) {
		...
		if (isLikelyStale) {
			throw new Error(
				...
				'any of the top 50 sorcerers on Vunira (unlikely).'
			);
		}
